...
$token = md5($YOUR_TOKEN_SALT.''.date('d/m/Y'));
if($_GET['token'] != $token){
...
class PHP_CRUD_API {
	...
	protected function createObject($input,$tables) {
		...
		//DSI - add you actions
		//return $tables[0]."<br>".$infos;
		/*$test = base64_encode($infos);
		
		
		// test si table : reaction (type , desc )  et get the Apps ID - si creation -
		$url='http://preprod.notifications.groupe-bel.com/notif/SendNotification.php?secret=dcdf51fcbcd2e9d443f2ceae574c366c06ab7455bf1658526141cac724d8379c&table='.$tables[0].'&value='.$test;
		include($url);
		//return $url;
		$ch = curl_init();
